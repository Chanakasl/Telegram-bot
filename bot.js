...
async function isBadWord(text) {
    ...
    try {
        const response = await axios.post("https://openrouter.ai/api/v1/chat/completions", {
            ...
            messages: [{ role: "system", content: "Is this toxic, profane or bad word in Sinhala or English? Reply ONLY YES or NO." }, { role: "user", content: text }]
...
bot.on('message', async (msg) => {
    ...
    if (cmd.includes('@')) cmd = cmd.split('@')[0]; // Extacts "/trending" from "/trending@botname"
    ...
    else if (cmd === '/movie' && query) { await sendSearchResults(chatId, query, 'movie', 1); }
    else if (cmd === '/tv' && query) { await sendSearchResults(chatId, query, 'tv', 1); }
    else if (cmd === '/actor' && query) { await sendActorSearchResults(chatId, query); }
    else if (cmd === '/year' && query) { 
        if (/^\d{4}$/.test(query)) await sendYearSearchResults(chatId, query, 1);
        else await bot.sendMessage(chatId, "⚠️ නිවැරදි වර්ෂයක් ඇතුලත් කරන්න. (Ex: /year 2024)");
    }
    else if (cmd === '/genres') {
        let kb = [
    ...
    else if (cmd === '/request' && query && ADMIN_CHAT_ID) {
